...
Does a simle in your online profile help your online micro-borrowing?(Athey et al, 2022 <small>NBER WP</small>)
...
Your **treatment effect** is the difference between thw two:
$$
\tau_i = Y_i(1) - Y_i(0)
$$
...
Athey and Imbens(2016): Tree-based models, which is an ML technique, can hint on how to choose $X_i$
...
2. Bias-Variance tradeoff in Prediction
    ...
    * Then, for a new data point $x$,
    $$
    \mathbb{E}[f(x)+\varepsilon-\hat{f}(x)] = \mathbb{E}[f-\hat{f}]^2 + \mathbb{V}(\hat{f}) + \mathbb{V}(\varepsilon)
    $$
...
![Figure from Beaulac and Rosenthal(2019)](figures/decision_tree_illustration.png){width=60%}

We do this by choosing $X_i$ and cutoff $k$ and minimize
...
### Forests
![Figure Source: Datacamp](figures/random_forest.png)
...
The two terms combined, we obtain unbiased estimator for honest target:
\begin{align*}
        \begin{aligned}
...
- Regressors: intercept(average) {\color{red} + dummy variable for treatment}
...
- Split Rule: Test $H_0$, the {\color{red}average treatment eﬀect} is the same in the two potential leaves, with $T^2$
