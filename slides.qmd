...
0. Split the data into $S^{tr}$(training) and $S^{te}$(test).
...
2. $\hat{y}(\text{leaf}) = \overline{y}_{(\text{leaf})} \quad$ (i.e. sample mean)

3. Compute the model performance in $S^{te}$.
...
* **Caution:** 
    ...
    + We compute the sample mean in $S^{est}$.
:::
